...
class Ce {
  ...
  toolCache;
  constructor(e, i, d) {
    this.abortController = void 0, this.config = ye, this.config.version = Ie, this.connectionConfig = e, this.tools = i, this.toolConfigs = d, this.toolCache = /* @__PURE__ */ new Map();
  }
  ...
  findObject(e, i) {
    for (const d in M)
      if (Object.prototype.hasOwnProperty.call(M, d) && M[d]?.find((c) => c.typeId === "object" && c.id === i.objectName))
  ...
  async previewObject(e, i) {
    try {
      ...
      const o = `${w}/fileStore${i.path}`, c = { Range: `bytes=0-${i.chunkSize ?? ze}` }, n = await fetch(encodeURI(o), { headers: c, signal: d });
      if (n.ok)
  ...
  async retrieveRecords(e, i, d, o) {
    const s = await e.loadTool("csv-parse");
    return console.log(1234, s), new Promise((c, n) => {
      try {
        e.abortController = new AbortController();
        ...
        a.addEventListener(
          "abort",
          () => {
            e.abortController = void 0, n(new h(je, "datapos-connector-file-store-emulator|Connector|retrieve.abort"));
          },
          { once: !0 }
        );
        let b = [];
  ...
  constructObjectNodeConfig(e, i, d, o, s) {
    const c = Q(d) ?? "", n = J(d), a = o, b = X(n);
    return { id: i, extension: n, folderPath: e, label: d, lastModifiedAt: a, mimeType: b, name: c, size: s, typeId: "object" };
  }
  // Helpers - Load tool.
  async loadTool(e) {
    const i = this.toolCache.get(e);
    if (i != null) return i;
    const d = `datapos-tool-${e}.es.js`, o = this.toolConfigs.find((a) => a.id === e);
    if (!o) throw new Error(`Unknown tool '${e}'.`);
    const c = await import(`https://engine-eu.datapos.app/tools/${d}_v${o.version}/datapos-tool-${d}.es.js`), n = new c.T();
    return this.toolCache.set(e, n), n;
  }
}
export {
  Ce as default
