...
const f = "https://sample-data-eu.datapos.app";
class te {
  abortController;
  config;
  engineUtilities;
  ...
  async auditObjectContent(t, i) {
    this.abortController = new AbortController();
    try {
      if (console.log(3333, t), t.parsingToolName === "datapos-tool-rust-csv-core") {
        const l = await this.getReadableStream({ id: "", path: t.path }), c = await m(this.toolConfigs, "rust-csv-core"), r = { delimiter: ",", hasHeaders: !0 }, b = t.supportsTransferableStreams ? await c.processWithTransferableStream(l, r, i) : await c.processWithChunks(l, r, i);
        return { processedRowCount: b.processedRowCount, durationMs: b.durationMs ?? 0 };
      }
      const d = await m(this.toolConfigs, "csv-parse"), s = { delimiter: t.valueDelimiterId, relax_column_count: !0, relax_quotes: !0 }, o = `${f}/fileStore${t.path}`, a = await d.parseStream(t, s, o, this.abortController, i);
  ...
  async previewObject(t) {
    const { signal: i } = this.abortController = new AbortController();
    try {
      const d = Date.now(), s = performance.now(), a = await (await m(this.toolConfigs, "file-operators")).previewFile(`${f}/fileStore${t.path}`, i, t.chunkSize);
      if (a.dataFormatId == null) throw new Error(`File '${t.path}' has unknown type.`);
      if (a.text == null) throw new Error(`File '${t.path}' is empty.`);
      const c = await (await m(this.toolConfigs, "csv-parse")).parseText(a.text, E), r = this.engineUtilities.inferDataTypes(c.parsedRecords);
      return {
        asAt: d,
  ...
  async retrieveRecords(t, i, d) {
    this.abortController = new AbortController();
    try {
      const s = await m(this.toolConfigs, "csv-parse"), o = { delimiter: t.valueDelimiterId, info: !0, relax_column_count: !0, relax_quotes: !0 }, a = `${f}/fileStore${t.path}`, l = await s.parseStream(t, o, a, this.abortController, i);
      d(l);
    } catch (s) {
      throw p(s);
    } finally {
