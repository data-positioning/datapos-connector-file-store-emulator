...
function v(t, e, i, d, s) {
  const a = s && "input" in s ? s.input : i.value, o = s?.expected ?? t.expects ?? null, r = s?.received ?? /* @__PURE__ */ w(a), n = {
    kind: t.kind,
    type: t.type,
    input: a,
    expected: o,
  ...
  }, l = t.kind === "schema", y = s?.message ?? t.message ?? /* @__PURE__ */ L(t.reference, n.lang) ?? (l ? /* @__PURE__ */ F(n.lang) : null) ?? d.message ?? /* @__PURE__ */ T(n.lang);
  ...
  return {
    ...
    "~run"(i, d) {
      ...
      for (const r of this.options) {
        ...
        if (n.typed) if (n.issues) a ? a.push(n) : a = [n];
        else {
          s = n;
          break;
        }
      ...
      if (a) {
        ...
        v(this, "type", i, d, { issues: /* @__PURE__ */ g(a) }), i.typed = !0;
      } else {
...
let h;
async function _() {
  return h ??= import("./datapos_connector_file_store_emulator_core-tvgNVoxR.js"), h;
}
async function ae(t, e) {
  const { add_numbers: i } = await _();
  return i(Math.trunc(t), Math.trunc(e));
}
async function ne(t) {
...
class be {
  ...
  async getReader(e, i) {
    try {
      ...
      const s = await this.versionChecksumUsingRust();
      return console.log("sum", s), await Promise.resolve({ readable: d.body });
    } catch (d) {
  ...
  async previewObject(e, i) {
    try {
      ...
      const s = `${E}/fileStore${i.path}`, a = { Range: `bytes=0-${i.chunkSize != null || re}` }, o = await fetch(encodeURI(s), { headers: a, signal: d });
      if (o.ok)
    ...
    return new Promise((a, o) => {
      try {
        ...
        l.on("readable", () => {
          ...
          try {
            r.throwIfAborted(), e.abortController = void 0, n.length > 0 && (d([]), n = []), s({
  ...
  async addUsingRust(e, i) {
    return ae(e, i);
  }
  async versionChecksumUsingRust() {
    return ne(this.config.version);
  }
  ...
  constructObjectNodeConfig(e, i, d, s, a) {
    const o = this.tools.dataPos.extractNameFromPath(d) ?? "", r = this.tools.dataPos.extractExtensionFromPath(d), n = s, l = this.tools.dataPos.lookupMimeTypeForExtension(r);
    return { id: i, extension: r, folderPath: e, label: d, lastModifiedAt: n, mimeType: l, name: o, size: a, typeId: "object" };
  }
}
export {
  be as default
