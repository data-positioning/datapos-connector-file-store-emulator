...
function z(t, e, i, d, o) {
  const s = o && "input" in o ? o.input : i.value, n = o?.expected ?? t.expects ?? null, c = o?.received ?? /* @__PURE__ */ R(s), a = {
    kind: t.kind,
    type: t.type,
    input: s,
    expected: n,
  ...
  return {
    ...
    "~run"(i, d) {
      ...
      if (s) {
        ...
        z(this, "type", i, d, { issues: /* @__PURE__ */ g(n) });
      }
      return i;
    }
  };
...
const l = (t) => new Map(Object.entries(t));
l({ "en-gb": "Delimited Text" }), l({ "en-gb": "Entity/Event" }), l({ "en-gb": "JSON Array" }), l({ "en-gb": "SPSS" }), l({ "en-gb": "XLS" }), l({ "en-gb": "XLSX" }), l({ "en-gb": "XML" });
...
const ue = "Connector failed to abort preview object operation.", je = "Connector failed to abort retrieve all records operation.", ze = 4096, ve = 1e3, w = "https://sample-data-eu.datapos.app";
class Ce {
  abortController;
  config;
  connectionConfig;
  ...
  findObject(e, i) {
    for (const d in M)
      if (Object.prototype.hasOwnProperty.call(M, d) && M[d]?.find((n) => n.typeId === "object" && n.id === i.objectName))
  ...
  async previewObject(e, i) {
    try {
      ...
      const o = `${w}/fileStore${i.path}`, n = { Range: `bytes=0-${i.chunkSize ?? ze}` }, c = await fetch(encodeURI(o), { headers: n, signal: d });
      if (c.ok)
        return e.abortController = void 0, { data: new Uint8Array(await c.arrayBuffer()), typeId: "uint8Array" };
      throw await K(c, `Failed to fetch '${i.path}' file.`, "datapos-connector-file-store-emulator|Connector|preview");
    } catch (d) {
      throw e.abortController = void 0, d;
    }
  }
  ...
  async retrieveRecords(e, i, d, o) {
    const s = e.csvParseTool ?? (e.csvParseTool = await e.loadTool("csv-parse"));
    return console.log(1234, s), new Promise((n, c) => {
      try {
        e.abortController = new AbortController();
        ...
        a.addEventListener(
          "abort",
          () => {
            e.abortController = void 0, c(new h(je, "datapos-connector-file-store-emulator|Connector|retrieve.abort"));
          },
          { once: !0 }
        );
        let b = [];
        const r = s.buildParser({
        ...
        r.on("readable", () => {
          try {
            let p;
            for (; (p = r.read()) !== null; )
              a.throwIfAborted(), b.push(p), !(b.length < ve) && (d(b), b = []);
          } catch (p) {
            e.abortController = void 0, c(y(p));
          }
        }), r.on("error", (p) => {
          e.abortController = void 0, c(y(p));
        }), r.on("end", () => {
          try {
            a.throwIfAborted(), e.abortController = void 0, b.length > 0 && (d(b), b = []), o({
            ...
            if (p.ok && p.body) {
              ...
              for (; !u.done; )
                a.throwIfAborted(), r.write(u.value, (C) => {
                  C && (e.abortController = void 0, c(y(C)));
                }), u = await v.read();
              r.end();
            } else {
              ...
              e.abortController = void 0, c(I);
            }
          } catch (I) {
            e.abortController = void 0, c(y(I));
          }
        }).catch((p) => {
          e.abortController = void 0, c(y(p));
        });
      } catch (a) {
        e.abortController = void 0, c(y(a));
      }
    });
  }
  /** Utilities - Construct folder node configuration. */
  constructFolderNodeConfig(e, i, d) {
    return { id: U(), childCount: d, folderPath: e, label: i, name: i, typeId: "folder" };
  }
  /** Utilities - Construct object (file) node configuration. */
  constructObjectNodeConfig(e, i, d, o, s) {
    const n = Q(d) ?? "", c = J(d), a = o, b = X(c);
    return { id: i, extension: c, folderPath: e, label: d, lastModifiedAt: a, mimeType: b, name: n, size: s, typeId: "object" };
  }
  // Helpers - Load tool.
  async loadTool(e) {
    const i = `datapos-tool-${e}.es.js`, d = this.toolConfigs.find((n) => n.id === e);
    if (!d) throw new Error(`Unknown tool '${e}'.`);
    const s = await import(`https://engine-eu.datapos.app/tools/${i}_v${d.version}/datapos-tool-${i}.es.js`);
    return new s.T();
  }
}
export {
