...
async function h(e, t, i) {
  ...
  try {
    ...
    o = `<body unavailable: ${m(a).message}>`;
  }
  return new u(s, i, o);
}
function m(e) {
...
class te {
  ...
  async auditObjectContent(t, i) {
    ...
    try {
      if (t.parsingToolName === "datapos-tool-rust-csv-core") {
        const l = await this.getReadableStream({ id: "", path: t.path }), c = await p(this.toolConfigs, "rust-csv-core"), r = { delimiter: ",", hasHeaders: !0 }, b = t.supportsTransferableStreams ? await c.processWithTransferableStream(l, r, i) : await c.processWithChunks(l, r, i);
        return { processedRowCount: b.processedRowCount, durationMs: b.durationMs ?? 0 };
      }
      const d = await p(this.toolConfigs, "csv-parse"), s = { delimiter: t.valueDelimiterId, info: !0, relax_column_count: !0, relax_quotes: !0 }, o = `${f}/fileStore${t.path}`, a = await d.parseStream(t, s, o, this.abortController, i);
  ...
  async previewObject(t) {
    ...
    try {
      ...
      const c = await (await p(this.toolConfigs, "csv-parse")).parseText(a.text, E), r = this.engineUtilities.inferDataTypes(c.parsedRecords);
      return {
        asAt: d,
  ...
  async retrieveRecords(t, i, d) {
    this.abortController = new AbortController();
    try {
      const s = await p(this.toolConfigs, "csv-parse"), o = { delimiter: t.valueDelimiterId, info: !0, relax_column_count: !0, relax_quotes: !0 }, a = `${f}/fileStore${t.path}`, l = await s.parseStream(t, o, a, this.abortController, i);
      d(l);
    } catch (s) {
      throw m(s);
    } finally {
