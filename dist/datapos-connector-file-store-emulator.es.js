...
class Ce {
  ...
  // Helpers - Load tool.
  async loadTool(e) {
    const i = `datapos-tool-${e}`, d = this.toolConfigs.find((n) => n.id === i);
    if (!d) throw new Error(`Unknown tool '${e}'.`);
    const s = await import(`https://engine-eu.datapos.app/tools/${i}_v${d.version}/${i}.es.js`);
    return new s.T();
  }
}
export {
