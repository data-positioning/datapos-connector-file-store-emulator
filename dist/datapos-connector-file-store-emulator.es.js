...
async function w(t, e) {
  const i = `datapos-tool-${e}`, d = t.find((s) => s.id === i);
...
class R extends E {
  ...
  constructor(e, i, d, o) {
    super(e, i, o), this.name = new.target.name, this.body = g(d ?? void 0);
  }
}
class y extends j {
}
...
async function $(t) {
  const { checksum_from_rust: e } = await v();
  return e(t);
}
async function v() {
  return z ??= import("./datapos_connector_file_store_emulator_core-CBVDrOML.js"), z;
}
const ee = "Connector failed to abort preview object operation.", te = "Connector failed to abort retrieve all records operation.", ie = 4096, de = 1e3, m = "https://sample-data-eu.datapos.app";
class se {
  abortController;
  config;
  connectionConfig;
  ...
  async getReadableStream(e, i) {
    try {
      ...
      const o = await W(12, 56), s = await $(e.config.version);
  ...
  listNodes(e, i) {
    const o = M[i.folderPath] ?? [], s = [];
    for (const a of o)
      a.typeId === "folder" ? s.push(this.constructFolderNodeConfig(i.folderPath, a.name, a.childCount)) : s.push(this.constructObjectNodeConfig(i.folderPath, a.id, a.name, a.lastModifiedAt, a.size));
  ...
  async previewObject(e, i) {
    try {
      ...
      d.addEventListener("abort", () => {
        throw new y(ee, "datapos-connector-file-store-emulator|Connector|preview.abort");
      });
      const o = { Range: `bytes=0-${i.chunkSize ?? ie}` }, s = await fetch(encodeURI(`${m}/fileStore${i.path}`), { headers: o, signal: d });
      if (s.ok)
  ...
  async retrieveRecords(e, i, d, o) {
    const s = await w(e.toolConfigs, "csv-parse");
    return new Promise((a, n) => {
      let c = !1;
      const r = (l) => {
        c || (c = !0, e.abortController && e.abortController.abort(), e.abortController = void 0, n(I(l)));
      };
      try {
        e.abortController = new AbortController();
        const l = e.abortController.signal;
        l.addEventListener("abort", () => r(new y(te, "retrieveRecords.abort")), { once: !0 });
        const u = i.chunkSize ?? de, b = this.createRowBuffer(d, u), f = s.buildParser({ delimiter: i.valueDelimiterId, info: !0, relax_column_count: !0, relax_quotes: !0 });
  ...
  createRowBuffer(e, i) {
    ...
    return { flush: o, push: (a) => {
      d.push(a), d.length >= i && o();
    } };
  }
  handleReadable(e, i, d, o) {
    try {
      let s;
      for (; (s = e.read()) !== null; )
        i.throwIfAborted(), d.push(s);
    } catch (s) {
      o(s);
    }
  }
  ...
  constructFolderNodeConfig(e, i, d) {
    return { id: h(), childCount: d, extension: void 0, folderPath: e, label: i, name: i, typeId: "folder" };
  }
  /** Construct object (file) node configuration. */
  constructObjectNodeConfig(e, i, d, o, s) {
    const a = P(d) ?? "", n = _(d), c = o, r = S(n);
