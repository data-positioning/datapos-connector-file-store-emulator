...
function M(t, e, i, d, n) {
  const s = n && "input" in n ? n.input : i.value, o = n?.expected ?? t.expects ?? null, l = n?.received ?? /* @__PURE__ */ C(s), a = {
    kind: t.kind,
    type: t.type,
    input: s,
    expected: o,
  ...
  }, c = t.kind === "schema", f = n?.message ?? t.message ?? /* @__PURE__ */ T(t.reference, a.lang) ?? (c ? /* @__PURE__ */ S(a.lang) : null) ?? d.message ?? /* @__PURE__ */ R(a.lang);
  f !== void 0 && (a.message = typeof f == "function" ? f(a) : f), c && (i.typed = !1), i.issues ? i.issues.push(a) : i.issues = [a];
}
// @__NO_SIDE_EFFECTS__
function h(t) {
  return {
...
}, ee = "0.2.272", te = "Connector failed to abort preview object operation.", ie = "Connector failed to abort retrieve all records operation.", de = 4096, oe = 1e3, u = "https://sample-data-eu.datapos.app";
class se {
  abortController;
  config;
  connectionConfig;
  ...
  listNodes(e, i) {
    const d = I[i.folderPath] ?? [], n = [];
    for (const s of d)
      s.typeId === "folder" ? n.push(this.constructFolderNodeConfig(i.folderPath, s.name, s.childCount)) : n.push(this.constructObjectNodeConfig(i.folderPath, s.id, s.name, s.lastModifiedAt, s.size));
  ...
  async previewObject(e, i) {
    try {
      ...
      const n = `${u}/fileStore${i.path}`, s = { Range: `bytes=0-${i.chunkSize != null || de}` }, o = await fetch(encodeURI(n), { headers: s, signal: d });
      if (o.ok)
      ...
      try {
        ...
        c.on("readable", () => {
          try {
            let r;
            for (; (r = c.read()) !== null; )
              l.throwIfAborted(), a.push(r), !(a.length < oe) && (d(a), a = []);
          } catch (r) {
          ...
          try {
            l.throwIfAborted(), e.abortController = void 0, a.length > 0 && (d(a), a = []), n({
  ...
  constructObjectNodeConfig(e, i, d, n, s) {
    const o = this.tools.dataPos.extractNameFromPath(d) ?? "", l = this.tools.dataPos.extractExtensionFromPath(d), a = this.tools.dataPos.convertMillisecondsToTimestamp(n), c = this.tools.dataPos.lookupMimeTypeForExtension(l);
    return { id: i, extension: l, folderPath: e, label: d, lastModifiedAt: a, mimeType: c, name: o, size: s, typeId: "object" };
  }
}
export {
  se as default
