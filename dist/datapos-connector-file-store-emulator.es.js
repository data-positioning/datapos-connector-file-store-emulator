...
async function y(t, e, i) {
  ...
  try {
    ...
    s = `<body unavailable: ${b(a).message}>`;
  }
  return new E(o, i, s);
}
function b(t) {
...
const $ = "Connector failed to abort preview object operation.", ee = "Connector failed to abort retrieve all records operation.", te = 4096, ie = 1e3, I = "https://sample-data-eu.datapos.app";
class de {
  abortController;
  config;
  connectionConfig;
  toolConfigs;
  constructor(e, i) {
    this.abortController = void 0, this.config = J, this.config.version = X, this.connectionConfig = e, this.toolConfigs = i;
  }
  ...
  findObjectFolderPath(e, i) {
    const d = M;
    for (const o in d)
      if (Object.hasOwn(d, o) && d[o]?.find((c) => c.typeId === "object" && c.id === i.nodeId))
  ...
  async previewObject(e, i) {
    try {
      ...
      d.addEventListener("abort", () => {
        throw new A($, "datapos-connector-file-store-emulator|Connector|preview.abort");
      });
      const o = { Range: `bytes=0-${i.chunkSize ?? te}` }, s = await fetch(encodeURI(`${I}/fileStore${i.path}`), { headers: o, signal: d });
      if (s.ok)
        return e.abortController = void 0, { data: new Uint8Array(await s.arrayBuffer()), typeId: "uint8Array" };
      throw await y(s, `Failed to fetch '${i.path}' file.`, "datapos-connector-file-store-emulator|Connector|preview");
    } catch (d) {
      throw e.abortController = void 0, d;
    }
  }
  ...
  async streamIntoParser(e, i, d, o, s) {
    ...
    if (!a.ok || !a.body)
      throw await y(a, `Failed to fetch '${i}' file.`, "datapos-connector-file-store-emulator|Connector|retrieve");
    const c = a.body.pipeThrough(new TextDecoderStream(d)).getReader();
    for (; ; ) {
      const { value: p, done: r } = await c.read();
      if (r) break;
  ...
  constructObjectNodeConfig(e, i, d, o, s) {
    const a = R(d) ?? "", c = P(d), p = o, r = _(c);
    return { id: i, extension: c, folderPath: e, label: d, lastModifiedAt: p, mimeType: r, name: a, size: s, typeId: "object" };
  }
}
export {
  de as default
