  ...
  return {
    ...
    "~run"(i, d) {
      ...
      if (s) {
        ...
        j(this, "type", i, d, { issues: /* @__PURE__ */ g(n) });
      }
      return i;
    }
  };
...
class Ce {
  ...
  async getReadableStream(e, i) {
    try {
      ...
      console.log("sum", s, o);
      const n = await e.loadCSVParseTool();
      return console.log(7777, n), await Promise.resolve({ readable: d.body });
    } catch (d) {
  ...
  async previewObject(e, i) {
    try {
      ...
      const o = `${w}/fileStore${i.path}`, n = { Range: `bytes=0-${i.chunkSize ?? je}` }, c = await fetch(encodeURI(o), { headers: n, signal: d });
      if (c.ok)
  ...
  async retrieveRecords(e, i, d, o) {
    const s = await e.loadCSVParseTool();
    return new Promise((n, c) => {
      try {
        ...
        a.addEventListener(
          "abort",
          () => {
            e.abortController = void 0, c(new h(ve, "datapos-connector-file-store-emulator|Connector|retrieve.abort"));
          },
          { once: !0 }
        );
        let b = [];
