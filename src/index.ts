...
const preview = (connector: Connector, dataViewConfig: DataViewConfig, chunkSize?: number): Promise<{ error?: unknown; result?: Preview }> => {
    return new Promise((resolve, reject) => {
        try {
            ...
            const url = `${URL_PREFIX}fileStore${dataViewConfig.folderPath}${dataViewConfig.objectName}`;
...
const read = (
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const url = `${URL_PREFIX}fileStore${dataViewConfig.folderPath}${fullFileName}`;
